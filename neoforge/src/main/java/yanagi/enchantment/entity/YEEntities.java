...
public class YEEntities {
	...
	// 	DeferredHolder<EntityType<?>, EntityType<E>> ret = ENTITY_TYPES.register(name, () -> builder.build(idOf(name)));
	...
	// 	DeferredHolder<EntityType<?>, EntityType<E>> ret = ENTITY_TYPES.register(name, () -> builder.build(idOf(name)));
	// 	SPAWN_EGGS.register(name + "_spawn_egg", () -> new SpawnEggItem(ret.get(), new Item.Properties().setId(idOf(name + "_spawn_egg"))));
	// 	ATTRIBUTES.put(ret, attributes);
	// 	if (predicate != null) {
	// 		SPAWN_PREDICATES.put(ret, predicate);
	// 	}
